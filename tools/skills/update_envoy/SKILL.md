...
2. Diagnosing and resolving any integration errors or merge conflicts.
3. Finalizing formatting, committing, and pushing the update branch to GitHub.
...
2. `GET_ENVOY_SHA`
3. `SET_NIGHTHAWK_BAZEL_DEP`
4. `COPY_EXACT_FILES`
5. `PATCH_SHARED_FILES`
6. `BAZEL_UPDATE_REQUIREMENTS` (`./ci/do_ci.sh fix_requirements`)
7. `BUILD_NIGHTHAWK` (`./ci/do_ci.sh build`)
8. `TEST_NIGHTHAWK` (`./ci/do_ci.sh test`)
9. `UPDATE_CLI_README` (`./ci/do_ci.sh fix_docs`)
10. `FIX_FORMAT` (`./ci/do_ci.sh fix_format`)
11. `GIT_ADD_INTEGRATION`
